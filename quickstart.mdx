...
```
  ...
  <Accordion icon="key" title="Configure the client">
    You can use LLM7.io without a key for low-volume use. For higher limits, get a free token at <a href="https://token.llm7.io/">token.llm7.io</a>.
...
<Tip><code>model="default"</code> is a smart router. Request a specific model later if you need strict pinning.</Tip> </Accordion>
...
<Tip>Responses return a CDN URL you can open in the browser or embed directly.</Tip> </Accordion>
...
<Note>
  **Need help?** Check the API reference above or open an issue in your docs repo.
  Tips and examples are added regularly.
</Note>
